...
hn::Vec<hn::ScalableTag<uint16_t>> WidenMulAccTo16eb(
...
hn::Vec<hn::ScalableTag<uint32_t>> WidenMulAccTo32eb (
...
size_t varint_decode_hwy_portable_early_branch(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
    using D8  = hn::ScalableTag<uint8_t>;
    const D8 d8;
    using V8  = hn::Vec<D8>;
    using M8  = hn::Mask<D8>;
    ...
    while (length >= N8) {
        ...
        // Early branching to reduce unnecessary ops, leads to different branches
        // doing initialization and storing. The compiler can not proove the 
        // initialization. These const conditions should be used when initializing 
        // and storing to make it easier to verify the variable has been initialized.
        const bool num_varints_u32_1 = num_varints >   N8/4;
        const bool num_varints_u32_2 = num_varints > 2*N8/4;
        const bool num_varints_u32_3 = num_varints > 3*N8/4;
        ...
        if (num_varints == N8) {
            ...
            V16 result12_16_lo = WidenMulAccTo16eb(
                    b2, 
                    hn::Set(d8, 128), 
                    b1_16_lo, 
                    b1_16_up, 
            ...
            if (count3 == 0) {
                ...
                if (num_varints_u32_1) {
                    StoreN(result12_loup, d32, output +   N8/4, num_varints -   N8/4);
                }
                if (num_varints_u32_2) {
                    StoreN(result12_uplo, d32, output + 2*N8/4, num_varints - 2*N8/4);
                }
                if (num_varints_u32_3) {
                    StoreN(result12_upup, d32, output + 3*N8/4, num_varints - 3*N8/4);
                }
                ...
                V16 result34_16_lo = WidenMulAccTo16eb(
                        b4, 
                        hn::Set(d8, 128), 
                        b3_16_lo, 
                        b3_16_up,
                ...
                V32 result1234_32_lolo = WidenMulAccTo32eb(
                        result34_16_lo,
                        hn::Set(d16, 16384),
                        result12_32_lolo,
                        result12_32_loup,
                ...
                V32 result12_32_uplo;
                V32 result12_32_upup;
                V32 result1234_32_uplo;
                V32 result1234_32_upup;
                if (num_varints_u32_2) {
                    result12_32_uplo = hn::PromoteLowerTo(d32, result12_16_up); 
                    result12_32_upup = hn::PromoteUpperTo(d32, result12_16_up); 

                    result1234_32_uplo = WidenMulAccTo32eb(
                            result34_16_up,
                            hn::Set(d16, 16384),
                            result12_32_uplo,
                            result12_32_upup,
                    ...
                    result1234_32_upup = result12_32_upup; 
                }
                ...
                if (count5 > 0) {
                    ...
                    // WidenMulAccTo32eb can not be used because 2<<28 > uint16_max
                    ...
                    // Safety: is guaranteed to be initialized because initialization is behind the same const condition
                ...
                #pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
                if (num_varints_u32_1) {
                    StoreN(result1234_32_loup, d32, output +   N8/4, num_varints -   N8/4);
                }
                if (num_varints_u32_2) {
                    StoreN(result1234_32_uplo, d32, output + 2*N8/4, num_varints - 2*N8/4);
                }
                if (num_varints_u32_3) {
                    StoreN(result1234_32_upup, d32, output + 3*N8/4, num_varints - 3*N8/4);
                }
                #pragma GCC diagnostic pop
                output += num_varints;
            }
...
extern "C" {
    size_t call_varint_decode_hwy_portable_early_branch(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
        return HWY_STATIC_DISPATCH(varint_decode_hwy_portable_early_branch)(input, length, output);
    }
}
