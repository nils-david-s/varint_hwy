...
hn::Vec<hn::ScalableTag<uint16_t>> WidenMulAccTo16Nomask(
    ...
    hn::Vec<hn::ScalableTag<uint8_t>> b_masked = hn::IfThenElseZero(mask, b);

    V16 a16_lo = hn::PromoteLowerTo(d16, a);
    V16 a16_up = hn::PromoteUpperTo(d16, a);
    V16 b16_lo = hn::PromoteLowerTo(d16, b_masked);
    V16 b16_up = hn::PromoteUpperTo(d16, b_masked);

    sum1 = hn::MulAdd(a16_up, b16_up, sum1);
    return hn::MulAdd(a16_lo, b16_lo, sum0);
}

hn::Vec<hn::ScalableTag<uint32_t>> WidenMulAccTo32Nomask (
    ...
    hn::Vec<hn::ScalableTag<uint16_t>> b_masked = hn::IfThenElseZero(mask, b);

    V32 a32_lo = hn::PromoteLowerTo(d32, a);
    V32 a32_up = hn::PromoteUpperTo(d32, a);
    V32 b32_lo = hn::PromoteLowerTo(d32, b_masked);
    V32 b32_up = hn::PromoteUpperTo(d32, b_masked);

    sum1 = hn::MulAdd(a32_up, b32_up, sum1);
    return hn::MulAdd(a32_lo, b32_lo, sum0);
}
    
size_t varint_decode_hwy_portable_nomask(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
    ...
    while (length >= N8) {
        ...
        if (num_varints == N8) {
            ...
            V16 result12_16_lo = WidenMulAccTo16Nomask(
                    b2, 
                    hn::Set(d8, 128), 
                    b1_16_lo, 
                    b1_16_up, 
            ...
            if (count3 == 0) {
                ...
                V16 result34_16_lo = WidenMulAccTo16Nomask(
                        b4, 
                        hn::Set(d8, 128), 
                        b3_16_lo, 
                        b3_16_up,
                ...
                V32 result1234_32_lolo = WidenMulAccTo32Nomask(
                        result34_16_lo,
                        hn::Set(d16, 16384),
                        result12_32_lolo,
                        result12_32_loup,
                ...
                V32 result1234_32_uplo = WidenMulAccTo32Nomask(
                        result34_16_up,
                        hn::Set(d16, 16384),
                        result12_32_uplo,
                        result12_32_upup,
                ...
                if (count5 > 0) {
                    ...
                    // WidenMulAccTo32Nomask can not be used because 2<<28 > uint16_max
...
extern "C" {
    size_t call_varint_decode_hwy_portable_nomask(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
        return HWY_STATIC_DISPATCH(varint_decode_hwy_portable_nomask)(input, length, output);
    }
}
