...
hn::Vec<hn::ScalableTag<uint16_t, 1>> WidenMulAccNomask(
    ...
    return hn::MulAdd(
            hn::PromoteTo(dout, a),
            hn::PromoteTo(dout, b_masked),
            hn::PromoteTo(dout, sum));
}
hn::Vec<hn::ScalableTag<uint32_t, 2>> WidenMulAccNomask(
...
size_t varint_decode_hwy_riscv_nomask(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
    ...
    while (length > 0) {
        ...
        if (num_varints == num_viable_bytes) {
            ...
            V16M2 result12_16 = WidenMulAccNomask(b1, hn::Set(d8, 128), b2, mask_second_bytes_16);
            ...
            V16M2 result34_16 = WidenMulAccNomask(b3, hn::Set(d8, 128), b4, mask_fourth_bytes);


            // shift result12 left by 14 (multiply with 16384) and add it to result34
            V32M4 result_1234 = WidenMulAccNomask(result12_16, hn::Set(d16, 16384), result34_16, hn::PromoteMaskTo(d16, d8, mask_third_bytes));
...
extern "C" {
    size_t call_varint_decode_hwy_riscv_nomask(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
        return HWY_STATIC_DISPATCH(varint_decode_hwy_riscv_nomask)(input, length, output);
    }
}
