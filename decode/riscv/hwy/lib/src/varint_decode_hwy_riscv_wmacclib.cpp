...
hn::Vec<hn::ScalableTag<uint16_t, 1>> WidenMulAccWmacclib(
    ...
    using D16h = hn::Half<decltype(dout)>;
    const D16h d16h;
    using V16h = hn::Vec<hn::Half<decltype(dout)>>;
    
    hn::Vec<decltype(din)> b_masked = hn::IfThenElseZero(mask, b);
    V16h sum1 = hn::PromoteUpperTo(d16h, sum);
    V16h sum0 = hn::WidenMulAccumulate(d16h, a, b_masked, hn::PromoteLowerTo(d16h, sum), sum1); 
    return hn::ConcatLowerLower(dout, hn::ZeroExtendVector(dout, sum1), hn::ZeroExtendVector(dout, sum0)); 
}
hn::Vec<hn::ScalableTag<uint32_t, 2>> WidenMulAccWmacclib(
    ...
    using D32h = hn::Half<decltype(dout)>;
    const D32h d32h;
    using V32h = hn::Vec<hn::Half<decltype(dout)>>;

    hn::Vec<decltype(din)> b_masked = hn::IfThenElseZero(mask, b);
    V32h sum1 = hn::PromoteUpperTo(d32h, sum);
    V32h sum0 = hn::WidenMulAccumulate(d32h, a, b_masked, hn::PromoteLowerTo(d32h, sum), sum1); 
    return hn::ConcatLowerLower(dout, hn::ZeroExtendVector(dout, sum1), hn::ZeroExtendVector(dout, sum0)); 
}



size_t varint_decode_hwy_riscv_wmacclib(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
    ...
    while (length > 0) {
        ...
        if (num_varints == num_viable_bytes) {
            ...
            V16M2 result12_16 = WidenMulAccWmacclib(b1, hn::Set(d8, 128), b2, mask_second_bytes_16);
            ...
            V16M2 result34_16 = WidenMulAccWmacclib(b3, hn::Set(d8, 128), b4, mask_fourth_bytes);


            // shift result12 left by 14 (multiply with 16384) and add it to result34
            V32M4 result_1234 = WidenMulAccWmacclib(result12_16, hn::Set(d16, 16384), result34_16, hn::PromoteMaskTo(d16, d8, mask_third_bytes));
...
extern "C" {
    size_t call_varint_decode_hwy_riscv_wmacclib(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
        return HWY_STATIC_DISPATCH(varint_decode_hwy_riscv_wmacclib)(input, length, output);
    }
}
