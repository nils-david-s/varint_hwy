...
size_t varint_decode_hwy_portable(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
    using D8  = hn::ScalableTag<uint8_t>;
    const D8 d8;
    using V8  = hn::Vec<D8>;
    using M8  = hn::Mask<D8>;
...
extern "C" {
    size_t call_varint_decode_hwy_portable(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
        return HWY_STATIC_DISPATCH(varint_decode_hwy_portable)(input, length, output);
    }
}
