...
size_t varint_decode_hwy_riscv(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
...
extern "C" {
    size_t call_varint_decode_hwy_riscv(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
        return HWY_STATIC_DISPATCH(varint_decode_hwy_riscv)(input, length, output);
    }
}
