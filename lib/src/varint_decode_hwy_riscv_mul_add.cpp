...
hn::Vec<hn::ScalableTag<uint16_t, 1>> WidenMulAccMulAdd(
    ...
    return hn::MaskedAddOr(
            hn::PromoteTo(dout, sum),
            hn::PromoteMaskTo(dout, din, mask),
            hn::PromoteTo(dout, sum),
            hn::Mul(PromoteTo(dout, a),
                PromoteTo(dout,b)));
}
hn::Vec<hn::ScalableTag<uint32_t, 2>> WidenMulAccMulAdd(
...
size_t varint_decode_hwy_riscv_mul_add(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
    ...
    while (length > 0) {
        ...
        if (num_varints == num_viable_bytes) {
            ...
            V16M2 result12_16 = WidenMulAccMulAdd(b1, hn::Set(d8, 128), b2, mask_second_bytes_16);
            ...
            V16M2 result34_16 = WidenMulAccMulAdd(b3, hn::Set(d8, 128), b4, mask_fourth_bytes);


            // shift result12 left by 14 (multiply with 16384) and add it to result34
            V32M4 result_1234 = WidenMulAccMulAdd(result12_16, hn::Set(d16, 16384), result34_16, hn::PromoteMaskTo(d16, d8, mask_third_bytes));
...
extern "C" {
    size_t call_varint_decode_hwy_riscv_mul_add(const uint8_t* HWY_RESTRICT input, size_t length, uint32_t* HWY_RESTRICT output) {
        return HWY_STATIC_DISPATCH(varint_decode_hwy_riscv_mul_add)(input, length, output);
    }
}
