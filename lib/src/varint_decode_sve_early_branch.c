...
size_t varint_decode_arm_early_branch(const uint8_t *data, size_t length, uint32_t *output) {
    ...
    while(length > 0) {
        ...
        if (num_varints == vl) {
            ...
            if (count3 == 0) {
                ...
                svuint32_t result1234_32_1;
                svuint32_t result1234_32_2;
                svuint32_t result1234_32_3;                

                if (num_varints > vl/4) {
                    svuint32_t result34_mul_widen_1 = svmul_n_u32_x(svunpkhi(svunpklo(mask_third_bytes)), svunpkhi_u32(result34_16_0), 16384);
                    result1234_32_1 = svadd_u32_m(svunpkhi(svunpklo(mask_third_bytes)), svunpkhi_u32(result12_16_0), result34_mul_widen_1);
                }

                if (num_varints > 2*vl/4) {
                    svuint32_t result34_mul_widen_2 = svmul_n_u32_x(svunpklo(svunpkhi(mask_third_bytes)), svunpklo_u32(result34_16_1), 16384);
                    result1234_32_2 = svadd_u32_m(svunpklo(svunpkhi(mask_third_bytes)), svunpklo_u32(result12_16_1), result34_mul_widen_2);
                }
                if (num_varints > 3*vl/4) {
                    svuint32_t result34_mul_widen_3 = svmul_n_u32_x(svunpkhi(svunpkhi(mask_third_bytes)), svunpkhi_u32(result34_16_1), 16384);
                    result1234_32_3 = svadd_u32_m(svunpkhi(svunpkhi(mask_third_bytes)), svunpkhi_u32(result12_16_1), result34_mul_widen_3);
                }
